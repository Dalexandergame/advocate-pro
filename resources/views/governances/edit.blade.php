...
<body>
    <div class="templates col-6 m-auto p-4" style="z-index: 2; margin-top: 5%">
        ...
        @endif
        <form action="{{ route('govertemplates.update', $govertemplate->id)}}" enctype="multipart/form-data" method="post">
            @csrf
            @method('PATCH')
            ...
            <div class="form-group">
                <input type="text" name="title" class="form-control" placeholder="Tapez votre texte ici ..." value="{{ old('title') ?? $govertemplate->title }}">
            </div>
            <div class="form-group">
                <input type="text" name="tag" class="form-control" placeholder="#Tags" value="{{ old('tag') ?? $govertemplate->tag }}">
            </div>
            <div>
                ...
                <textarea id="editor" name="description" cols="81" rows="20" placeholder="Tapez votre texteici ...">{{ old('description') ?? $govertemplate->description }}</textarea>
            </div>
