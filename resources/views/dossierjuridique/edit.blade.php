...
<div class="container">
	<div class="big-grid" style="margin-right: -20px; height: 450px">
		<div class="row" style="margin-top: 20px">
			...
			<form action="{{ url('dossierjuridiques/'.$dossierjuridique->id) }}" method="post">
				...
				<div class="form-group col-md-7 type-move">
					...
					<select id="inputState" name="type_dossier" class="form-control">
						...
						<option value="type1"{{$dossierjuridique->type_dossier =="type1" ? 'selected' : ''}}>type 1</option>
						<option value="type2"{{$dossierjuridique->type_dossier =="type2" ? 'selected' : ''}}>type 2</option>
					</select>
				</div>
				<div class="form-row type-move2">
					<div class="col">
						<label for="">Pour</label>
						<input type="text" class="form-control" placeholder="Nom du compte" name="for" value="{{ $dossierjuridique->for }}">
					</div>
					<div class="col">
						<label for="">Contre</label>
						<input type="text" class="form-control" placeholder="Nom du compte" name="against" value="{{ $dossierjuridique->against }}">
					</div>
				</div>
				<div class="form-row type-move3">
					<div class="col">
						<label for="">Client direct</label>
						<input type="text" class="form-control" placeholder="Nom du compte" name="client_direct" value="{{ $dossierjuridique->client_direct }}">
					</div>
					<div class="col">
						<label for="">Client indirect</label>
						<input type="text" class="form-control" placeholder="Nom du compte" name="client_indirect" value="{{ $dossierjuridique->client_indirect }}">
					</div>
				</div>
				<div class="form-group text-area-move">
					<label for="exampleFormControlTextarea1">Commentaire principal</label>
					<textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="comments">{{ $dossierjuridique->comments }}</textarea>
				</div>
				<div class="form-row type-move4">
					<div class="col">
						<input type="text" class="form-control" placeholder="numero dossier" name="tribunal_number" value="{{ $dossierjuridique->tribunal_number }}">
					</div>
				</div>
				<input type="submit" name="enregistrer" value="modifier" class="buttonw">
			</form>
